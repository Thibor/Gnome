...
int mm_value[6] = { 94, 337, 365, 477, 1025, 0 };
...
static void Init() {
	...
	for (int pt = PAWN; pt <= KING; pt++) {
		for (int sq = 0; sq < 64; sq++) {
			...
			mm_pst[pt][sq] = mm_pst[pt + PT_NB][FLIP(sq)] = max(mg, eg);
		}
	}
}
...
static U64 GetHash(const Position* pos) {
	...
	for (int y = 0; y < 8; y++)
		for (int x = 0; x < 8; x++) {
			...
			if (piece)
				hash ^= keys[(piece & 0xf) * 64 + board64[sq]];
		}
	if (pos->ep < no_sq)
		hash ^= keys[6 * 64 + board64[pos->ep]];
	if (pos->castle)
...
static int EvalMove(Position* pos, Move* bst, Move* m) {
	...
	int score = mm_pst[piece][board64[m->to]] - mm_pst[piece][board64[m->from]];
	if ((m->from == bst->from) && (m->to == bst->to))
		score += 10000;
	if (m->promo < PT_NB)
		score += mm_value[m->promo] - mm_value[PAWN];
	if (pDes)
		score += 10 * mm_value[ptDes] - mm_value[ptSou];
	return score;
}
...
static int MakeMove(Position* pos, const Move* move) {
	...
	pos->castle &= boardCastle[board64[move->from]] & boardCastle[board64[move->to]];
